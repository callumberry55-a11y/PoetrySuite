...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!keyData) {
      await logUsage('unknown', '/external-api-contests', req.method, 401, req, Date.now() - startTime, supabase);
      return new Response(
    ...
    if (!keyData.permissions.read_contests) {
      await logUsage(keyData.id, '/external-api-contests', req.method, 403, req, Date.now() - startTime, supabase);
      return new Response(
        JSON.stringify({ error: 'This API key does not have permission to read contests' }),
        {
          status: 403,
          headers: {
            ...corsHeaders,
    ...
    if (!withinRateLimit) {
      await logUsage(keyData.id, '/external-api-contests', req.method, 429, req, Date.now() - startTime, supabase);
      return new Response(
    ...
    let query = supabase
      .from('contests')
      .select('id, title, description, start_date, end_date, status, prize_points, created_at')
    ...
    await logUsage(keyData.id, '/external-api-contests', req.method, 200, req, responseTime, supabase);
