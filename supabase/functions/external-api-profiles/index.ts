...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!keyData) {
      await logUsage('unknown', '/external-api-profiles', req.method, 401, req, Date.now() - startTime, supabase);
      return new Response(
    ...
    if (!keyData.permissions.read_public_profiles) {
      await logUsage(keyData.id, '/external-api-profiles', req.method, 403, req, Date.now() - startTime, supabase);
      return new Response(
        JSON.stringify({ error: 'This API key does not have permission to read profiles' }),
        {
          status: 403,
          headers: {
            ...corsHeaders,
    ...
    if (!withinRateLimit) {
      await logUsage(keyData.id, '/external-api-profiles', req.method, 429, req, Date.now() - startTime, supabase);
      return new Response(
    ...
    await logUsage(keyData.id, '/external-api-profiles', req.method, 200, req, responseTime, supabase);
